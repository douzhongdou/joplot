import type { TranslationDictionary } from '../types'

export const en = {
  common: {
    selectPlaceholder: 'Select',
    csvCount: ({ count }: { count: string | number }) => `${count} datasets`,
    rowCount: ({ count }: { count: string | number }) => `${count} rows`,
    on: 'On',
    off: 'Off',
    notAvailableYet: 'Not available yet',
  },
  language: {
    label: 'Language',
    options: {
      'zh-CN': '中文',
      en: 'English',
      'ja-JP': '日本語',
    },
  },
  chartKinds: {
    line: 'Line Chart',
    scatter: 'Scatter Plot',
    bar: 'Bar Chart',
    pie: 'Pie Chart',
    stats: 'Stats Card',
    area: 'Area Chart',
    radar: 'Radar Chart',
    heatmap: 'Heatmap',
    fallback: 'Card',
  },
  cards: {
    defaultLineTitle: 'Default Line Chart',
    copySuffix: 'Copy',
    fallbackSeriesLabel: 'Data Series',
    unnamedSeries: 'New Series',
  },
  uploader: {
    importTitle: 'Import Data',
    importDescription: 'Import one or more CSV or Excel files to start.',
    uploadCsv: 'Upload Data',
    addCsv: 'Add Data',
    overlayBadge: 'Data Upload',
    overlayTitle: 'Drop to upload one or more CSV or Excel files',
  },
  persistence: {
    storageWarning: 'The data exceeds the browser storage limit. Imported data will be lost after a page refresh, so keep a backup of the original files.',
  },
  errorBoundary: {
    title: 'Something went wrong',
    description: 'Some data may have triggered an unexpected error. Reload the page to restore your last saved workspace.',
    reload: 'Reload page',
  },
  workbench: {
    addComponent: 'Add component',
    filters: 'Filters',
    theme: 'Theme',
    currentGroup: 'Current group',
    range: 'Range',
    filtersTitle: 'Filter rules',
    filtersDescription: 'All imported datasets in the current workbench share the same filter logic.',
    joinAnd: 'All must match',
    joinOr: 'Any can match',
    addCondition: 'Add condition',
    resetData: 'Reset data',
    noFilters: 'No filters yet.',
    valuePlaceholder: 'Filter value',
    startValuePlaceholder: 'Start value',
    endValuePlaceholder: 'End value',
    removeFilter: 'Remove',
  },
  filterOperators: {
    contains: 'Contains',
    equals: 'Equals',
    gt: 'Greater than',
    lt: 'Less than',
    between: 'Between',
  },
  inspector: {
    emptyTitle: 'Select a chart card first',
    emptyDescription: 'Base settings and display settings will appear here.',
    duplicateCard: 'Duplicate card',
    deleteCard: 'Delete card',
    mobileOpenButton: 'Edit',
    mobileDrawerEyebrow: 'Card editor',
    closeDrawer: 'Close card editor',
    baseTab: 'Base settings',
    displayTab: 'Display settings',
    baseSectionTitle: 'Base settings',
    seriesSectionTitle: 'Data series',
    seriesDisplaySectionTitle: 'Series display',
    chartDisplaySectionTitle: 'Chart display',
    title: 'Title',
    chartKind: 'Chart type',
    dataMode: 'Data mode',
    dataModes: {
      raw: 'Raw data',
      aggregate: 'Aggregated data',
    },
    sharedXAxis: 'Shared X axis',
    drawMode: 'Draw mode',
    lineWidth: 'Line width',
    addSeries: 'Add series',
    seriesMenu: 'Series menu',
    openDisplaySettings: 'Open display settings',
    removeSeries: 'Remove series',
    chooseField: 'Choose field',
    incompatibleSeries: ({ xColumn }: { xColumn: string | number }) =>
      `This dataset does not include the shared X axis "${xColumn}", so the series will not render.`,
    dataSource: 'Data source',
    displayName: 'Display name',
    displayNamePlaceholder: 'Enter a display name',
    seriesColor: 'Color',
    legend: 'Legend',
    gridLines: 'Grid lines',
    axes: 'Axes',
    themeColor: 'Theme color',
    axisRangeSectionTitle: 'Axis range',
    xRangeMin: 'X axis start',
    xRangeMax: 'X axis end',
    yRangeMin: 'Y axis min',
    yRangeMax: 'Y axis max',
    autoRangePlaceholder: 'Auto',
    aggregate: {
      dataSources: 'Data sources',
      xColumn: 'X axis field',
      xKind: 'X axis type',
      timeBucket: 'Time bucket',
      metricColumn: 'Metric field',
      aggregation: 'Aggregation',
      groupMode: 'Grouping',
      groupColumn: 'Group field',
    },
    heatmap: {
      dataset: 'Dataset',
      xColumn: 'X axis field',
      yColumn: 'Y axis field',
      zColumn: 'Value field',
      zCount: 'Count (auto)',
    },
    xKinds: {
      category: 'Text / category',
      number: 'Number',
      time: 'Time',
    },
    timeBuckets: {
      day: 'Day',
      week: 'Week',
      month: 'Month',
      quarter: 'Quarter',
      year: 'Year',
    },
    groupModes: {
      file: 'Group by file',
      field: 'Group by field',
    },
    aggregations: {
      sum: 'Sum',
      mean: 'Average',
      count: 'Count',
      max: 'Max',
      min: 'Min',
      median: 'Median',
      distinctCount: 'Distinct count',
      missingCount: 'Missing count',
      stddev: 'Standard deviation',
      variance: 'Variance',
    },
  },
  drawModes: {
    lines: 'Straight Lines',
    spline: 'Smooth Curve',
    'lines+markers': 'Lines + markers',
    'spline+markers': 'Smooth Curve + markers',
  },
  chartCard: {
    dragCard: 'Drag chart card',
    seriesCount: ({ count }: { count: string | number }) => `${count} series`,
    stats: {
      dataset: 'Dataset',
      validValues: 'Valid values',
      missingValues: 'Missing values',
      min: 'Min',
      max: 'Max',
      mean: 'Mean',
    },
    autorange: 'Auto range',
    copyImage: 'Copy image',
    downloadImage: 'Download image',
    copySuccess: 'Copied',
    copyDownloadedFallback: 'Clipboard unavailable, downloaded instead',
    noValidSeries: 'This chart card does not have any valid series to render. Choose a usable dataset and field in the inspector.',
    aggregateSkipped: ({ datasets, rows, series }: { datasets: string | number; rows: string | number; series?: string | number }) =>
      `Skipped ${datasets} data sources, ${rows} rows, and ${series ?? 0} groups while aggregating.`,
    resizeCard: 'Resize chart card',
  },
  dataView: {
    chartLabel: 'Chart',
    tabLabel: 'Data',
    emptyState: 'Upload a data file first',
  },
  mobileNav: {
    actions: 'Actions',
    actionsEyebrow: 'Workbench',
    actionsTitle: 'Quick actions',
    closeActions: 'Close actions panel',
  },
  nav: {
    sectionsLabel: 'Sections',
    workbench: 'Data',
    function: 'Functions',
  },
  functionStudio: {
    heading: 'Function plotter',
    description: 'Type a formula in the form y = f(x) and the curve appears instantly. Overlay several curves, tune parameters with sliders, and asymptotes are split automatically.',
    functionsHint: 'Supports sin, cos, tan, exp, ln, log, sqrt, abs, floor, min, max and more, plus the constants pi and e. Implicit multiplication such as 2x or x(x+1) works too.',
    curvesSectionTitle: 'Curves',
    addCurve: 'Add curve',
    expressionLabel: 'Formula',
    expressionPlaceholder: 'e.g. sin(x) / x',
    colorLabel: 'Curve color',
    toggleCurve: 'Show / hide curve',
    removeCurve: 'Remove curve',
    invalidCurve: 'This formula cannot be parsed yet.',
    domainSectionTitle: 'Domain & sampling',
    xMin: 'X from',
    xMax: 'X to',
    yMin: 'Y min',
    yMax: 'Y max',
    autorangeHint: 'Clears the manual Y range and restores the adaptive view',
    samplesLabel: 'Sampling density',
    samplesValue: ({ count }: { count: string | number }) => `${count} points`,
    paramsSectionTitle: 'Parameter sliders',
    paramsEmptyHint: 'Use any letter other than x in a formula (such as a, b or k) and a slider for it shows up here.',
    paramMin: 'Min',
    paramMax: 'Max',
    examplesSectionTitle: 'Examples',
    examples: {
      damped: 'Damped oscillation',
      beats: 'Beats',
      gaussian: 'Gaussian',
      sigmoid: 'Sigmoid',
      sinc: 'Sinc',
      cubic: 'Cubic',
      tangent: 'Tangent',
      amplitude: 'Tunable amplitude',
    },
    sendToWorkbench: 'Send to workbench',
    sendHint: 'Adds the sampled points of every visible curve to the data workbench as datasets for comparison and export.',
    sendSuccess: ({ count }: { count: string | number }) => `Added ${count} dataset(s), opening the workbench…`,
    sendFailed: 'Could not send: browser storage is unavailable or full.',
    sendEmpty: 'There is no valid curve to send yet.',
    emptyPlotTitle: 'Nothing to plot yet',
    emptyPlotDescription: 'Enter a valid formula on the left or pick one of the examples.',
    errors: {
      empty: 'The formula is empty.',
      'unexpected-character': ({ token, position }: { token?: string | number; position?: string | number }) =>
        `Character “${token ?? ''}” at position ${Number(position ?? 0) + 1} is not recognized.`,
      'invalid-number': ({ token }: { token?: string | number }) => `The number “${token ?? ''}” is malformed.`,
      'unexpected-end': 'The formula ends early — an operand or closing parenthesis is probably missing.',
      'unexpected-token': ({ token, position }: { token?: string | number; position?: string | number }) =>
        `“${token ?? ''}” near position ${Number(position ?? 0) + 1} is not valid here.`,
      'unbalanced-parenthesis': 'Parentheses do not match — there is an extra closing parenthesis.',
      'function-needs-parentheses': ({ name }: { name?: string | number }) => `Write the function ${name ?? ''} as ${name ?? ''}(…).`,
      'wrong-argument-count': ({ name, expected, actual }: { name?: string | number; expected?: string | number; actual?: string | number }) =>
        `${name ?? ''} expects ${expected ?? ''} argument(s) but received ${actual ?? ''}.`,
    },
  },
  help: {
    label: 'Help',
    title: 'Shortcuts',
    boxSelect: {
      action: 'Drag to select',
      description: 'Box select to zoom into a region',
    },
    scrollZoom: {
      action: 'Scroll wheel',
      description: 'Scroll to zoom in or out',
    },
    middlePan: {
      action: 'Middle button drag',
      description: 'Hold middle button and drag to pan',
    },
    doubleClickReset: {
      action: 'Double-click',
      description: 'Double-click to reset the view',
    },
  },
} satisfies TranslationDictionary
